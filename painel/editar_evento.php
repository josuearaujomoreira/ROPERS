...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['nome'])) {
    ...
    $stmt = $pdo->prepare("UPDATE eventos SET nome = ?, data = ?, local = ?, imagem = ? WHERE id = ?");
    $stmt->execute([$nome, $data_evento, $local, $imagem, $id]);
...
<head>
    ...
    <style>
        body { background-color: #f5f6f6; min-height: 100vh; margin: 0; }
        .sidebar { width: 220px; transition: width 0.3s; background-color: #343434; color: #f5f6f6; min-height: 100vh; }
        ...
        .content { flex-grow: 1; padding: 20px; position: relative; overflow: hidden; }
